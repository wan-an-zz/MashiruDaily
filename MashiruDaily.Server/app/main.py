...
（大小写敏感 PascalCase）逐字节对齐；客户端启动时先拉 meta 比对 createdAt，
再拉全量列表整体替换本地数据——因此任何额外的键（如本地字段 HasSynced）都不得出现。
"""
...
def get_meta() -> dict:
    """返回元数据：date / createdAt 透传侧车，count 取 todo.json 实时长度。"""
    return current_meta()


@app.get("/api/todo")
def get_todo_list() -> list:
    """返回 todo.json 全量列表，PascalCase 逐字透传（不含 HasSynced）；缺失时返回空数组。"""
