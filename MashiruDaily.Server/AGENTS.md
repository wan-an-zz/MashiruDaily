...
两职责：① **拉取服务器**（只读 GET，默认 `0.0.0.0:8123`：`/health`、`/api/todo/meta`、`/api/todo`，数据源 `data/todo.json`）；② **Hermes 装配工具集**（`bootstrap.py` 一键串联 setup_server → register_hermes_plugin → configure_webhook → configure_cron → install_autostart）。开发采用 **TDD**：先写契约测试（RED）再实现转绿。
