...
- `MashiruDaily.Tui/`（Terminal.Gui v2 控制台，Linux 可运行）— 引用 **Core**（不是 `MashiruDaily`，避免拖入 Avalonia）。`Program.cs`（DI + 生命周期，注册 `TodoPageViewModel`+`TalkViewModel`）+ `Views/MainWindow`、`TodoColumnView`、`TodoRowView`、`AgentReactionView`（Tab 键循环切换 Todo 与 Agent 反应页面）。**先读 `MashiruDaily.Tui/AGENTS.md`**，里面是 Terminal.Gui v2 权威规范。
