...
- `Program.cs` — DI 装配（与 `MashiruDaily/App.axaml.cs` 同款 Core 注册：Todo 两件套 + 远程同步三件套 + HttpClient + `TodoPageViewModel`，**不含** Avalonia 专有的 `MainViewModel`/`SettingsPageViewModel`）+ 生命周期（`TodoService.InitializeAsync` → 即发即忘同步 → `Application.Create()/Init()/Run(MainWindow)` → Esc 退出冲刷）。
